    ...
    $(document).ready(function() {
        ...
        form.children(".wizard").steps({
            ...
            onFinished: function(event, currentIndex) {
                ...
                $.ajax({
                    url: base_url+"/addSucursal/registrar",
                    type: "post",
                    dataType: "json",
                    data: formData,
                    cache: false,
                    ...
                    success: function(respuesta){
                        if(respuesta.rows.success == true){
                            bootbox.alert({
                                message: "<h3><i class='fas fa-check-circle' style='color:#13b313'></i> Sucursal agregada exitosamente.</h3>"+
                                "<h5>El usuario para iniciar sesion es: "+respuesta.rows.accountResponse.email+"</h5>",
                                locale: 'mx'
                            });
                            setTimeout("window.location.href = '"+base_url+"/addSucursal'", 3000);
                        }else{
                            bootbox.alert({
