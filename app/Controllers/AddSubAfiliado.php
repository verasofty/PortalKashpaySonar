...
class AddsubAfiliado extends BaseController{
	...
	public function index(){
		if (session('idRol') == 2) {
			...
			return view('addSubAfiliado/index',$datos);
		}else{
    ...
    $arrayRep2 = array();
      ...
      $amount=$_POST['monto'];
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => WS_KASPAYSERVICES.'/portalKashPayServices/api/v1/subAffiliation',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
		...
		if($datos['rows']->success){
			...
			//$guid = 'c42937db-7601-4202-abc6-231323587db3';
			$filesave;
