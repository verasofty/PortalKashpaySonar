...
class Spei extends BaseController{
	...
	public function index(){
       ...
       $curl_cuentas = curl_init();
		...
		$datos = array('contactos'=>json_decode($response),'contactosUsa'=>json_decode($response_usa),'cuentas'=>json_decode($response_cuentas));

		if (session('idUser')) {
			return view('spei/index', $datos);
		}else{
    ...
    $cuentaOr = curl_escape($curl, $_POST['cuentaOr']);


		curl_setopt_array($curl, array(
	      CURLOPT_URL => WS_SALDOS.'/Entities/entities/'.$cuentaOr.'/operations',
	      CURLOPT_RETURNTRANSFER => true,
	      CURLOPT_ENCODING => '',
	      CURLOPT_MAXREDIRS => 10,
	      CURLOPT_TIMEOUT => 0,
  ...
  public function envioSMS(){
		...
		curl_setopt_array($curl, array(
			  CURLOPT_URL => WS_KASPAYSERVICES.'/portalKashPayServices/api/v1/user/sendOperationTokenBySMS',
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => '',
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
	...
	public function validarToken(){
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => WS_KASPAYSERVICES.'/portalKashPayServices/api/v1/user/validateOperationWithSMSToken?sirioId='.session('entitySonID').'&idOperationType=1&token='.$_POST['token'],
			CURLOPT_RETURNTRANSFER => true,
		  	CURLOPT_ENCODING => '',
		  	CURLOPT_MAXREDIRS => 10,
		  	CURLOPT_TIMEOUT => 0,
