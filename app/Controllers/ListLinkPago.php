...
class ListLinkPago extends BaseController{
	...
	public function index(){
		...
		if (session('idUser')) {
			...
			if (session('idRol') == 2) {
				...
				$datos= array('subafiliado'=>json_decode($response_subafiliado));
			...
			return view('listLinkPago/index',$datos);
		}else{
			return redirect()->to(base_url().'/');
		}
	}
	public function searchLinkPago(){
		$curl = curl_init();

		curl_setopt_array($curl, array(
		CURLOPT_URL =>WS_KASPAYSERVICES. '/portalKashPayServices/api/v1/paymentLink/findBy?idContext='.$_POST['subafiliado'].'&idEntity='.$_POST['entidad'].'&idTerminal='.$_POST['sucursal'].'&idTerminalUser='.$_POST['caja'].'&amount='.$_POST['monto'].'&reference='.$_POST['referencia'].'&date='.$_POST['fechaInicio'],
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => '',
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 0,
		...
		CURLOPT_HTTPHEADER => array(
			'Authorization: Basic YWRtaW46c2VjcmV0',
			'Cookie: JSESSIONID=4dbacb2e28fa788bcf9d4ddf3c2d'
		),
		));
