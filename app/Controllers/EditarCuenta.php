...
class Editarcuenta extends BaseController{
	...
	public function index(){
		//if (session('idRol') == 3) {
			...
			$datos = array('subafiliados'=> json_decode($responseSubafiliado),'giros'=>json_decode($response),'regimenFiscal'=>json_decode($response_regimenFiscal));
		
		//var_dump($responseCuenta);
		if (session('idUser')!='') {
			return view('editarCuenta/index',$datos);
		}else{
  ...
  public function updateCuenta(){
	...
	public function searchDireccion($idLocalidad){

		$curl = curl_init();

		curl_setopt_array($curl, array(
		  CURLOPT_URL => WS_KASPAYSERVICES.'/portalKashPayServices/api/v1/localidades/getByIdLocation?idLocation='.$idLocalidad,
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => '',
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 0,
		  ...
		  CURLOPT_HTTPHEADER => array(
		    'Authorization: Basic YWRtaW46c2VjcmV0',
		    'Cookie: JSESSIONID=030345758b2f8e90e3d22bc41a4d'
		  ),
		));
