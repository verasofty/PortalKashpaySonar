...
class AddEntidad extends BaseController{
	...
	public function index(){
		...
		if (session('idRol') == 2 || session('idRol') == 3 || session('idRol') == 7) {
			return view('addEntidad/index',$datos);
		}else{
	...
	public function searchComisionista ($subafiliado){
		...
		$curl_comisionista = curl_init();

		curl_setopt_array($curl_comisionista, array(
		...
		$response = curl_exec($curl_comisionista);

		curl_close($curl_comisionista);
