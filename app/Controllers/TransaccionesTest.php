...
class TransaccionesTest extends BaseController{
	...
	public function index(){
		...
		if (session('idUser')) {
			return view('transaccionesTest/index', $datos);
		}else{
   ...
   $curl = curl_init();
		...
		echo WS_KASPAYSERVICES.'/portalKashPayServices/api/v1/operations/searchOperations?idContext='.$_POST['subafiliado'].'&idEntity='.$_POST['entidad'].'&idTerminal='.$_POST['sucursal'].'&idTerminalUser='.$_POST['caja'].'&typeOperation='.$_POST['operacion'].'&card='.$_POST['numTarjeta'].'&amount='.$_POST['monto'].'&responseCode='.$_POST['edoTransaccion'].'&referenceNumber='.$_POST['referencia'].'&authorizationNumber='.$_POST['autorizacion'].'&bin='.$_POST['bin'].'&startDate='.$location.'&endDate='.$location2;
